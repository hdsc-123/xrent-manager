...
│   │   ├── payments.ts      # (Sprint 6) CRUD + recomputeInvoiceStatus (recalcule toujours amountPaid/status de la facture depuis les paiements réels) ; (Sprint 14B) PaymentExceedsRemainingBalanceError toujours formatée en devise (jamais un entier brut de centimes) ; (Sprint 17, nouveau) createMixedPayments — paiement mixte atomique depuis la fiche facture existante (revalide le solde réel juste avant d'écrire, même garantie que processLocationPayment), exposée via POST /api/payments (champ optionnel lines)
...
│       ├── csv-export-sanitization.test.ts # (Sprint 16, nouveau) Test unitaire pur (sans serveur next dev) de sanitizeCsvCell (ExportCsvButton.tsx) — protection injection de formule CSV
