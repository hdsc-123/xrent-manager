...
│   │   ├── permissions.ts   # (Sprint 12C) catalogue PERMISSIONS (code, pas de table) + groupes par défaut + can()/getEffectivePermissions() + CRUD PermissionGroup + assignation par user ; (Sprint 15) catalogue étendu (maintenances/alerts/cash_register/vehicle_transfers/vehicle_trips), DEFAULT_GROUPS révisés (COMPTABILITÉ resserré, MEMBER/AGENCE complétés pour préserver le comportement antérieur), SPRINT15_BACKFILL_PERMISSIONS (rétro-application aux groupes déjà existants d'un tenant), getEffectivePermissions() replie désormais un User sans permissionGroupId sur les permissions du groupe MEMBER par défaut (jamais un ensemble vide) — voir DOMAINRULES.md section 22
