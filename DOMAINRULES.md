...
- **Décision validée et implémentée (Sprint 10)** : un utilisateur peut modifier son propre profil (nom, email, mot de passe) via `GET`/`PATCH /api/users/me` — toujours scopé à l'user de la session, aucun id cible n'est jamais accepté depuis le client. Un changement de mot de passe exige la vérification du mot de passe actuel (`currentPassword`), contrairement à la réinitialisation *par un ADMIN sur un autre user* (section 4, sans cette vérification — action administrative distincte). Un changement d'email est refusé (409) s'il entrerait en collision avec un email déjà utilisé dans le même tenant. Aucune invalidation/rafraîchissement des sessions JWT existantes sur d'autres appareils n'est déclenché par ce changement — **À DÉCIDER** si jugé nécessaire (voir [SECURITY.md](./SECURITY.md) section 5).
