...
Dernière mise à jour : 2026-08-12 — Sprint 8 (module clients dédié + consolidation métier/sécurité) validé : page `/dashboard/clients*` complète (liste + recherche, création, détail/édition, suppression bloquée si location associée — même principe que `deleteVehicle`), CRUD API `/api/clients*` complet (`GET`/`POST` existaient depuis Sprint 5, `GET`/`PATCH`/`DELETE /api/clients/[id]` nouveaux), tenant-scopé sans notion d'agence (confirme l'hypothèse de travail DOMAINRULES.md section 9), aucune régression de sécurité trouvée lors de la relecture (voir section 6), 146/146 tests passés au total.

## 1. État actuel

Le projet est au stade **Sprint 8 — module clients dédié et consolidation métier/sécurité**. Le dépôt contient désormais, en plus des sprints précédents :
...
Le module clients dédié étant en place (Sprint 8), en plus de la maintenance/alertes (Sprint 7), la facturation/paiements/rapports (Sprint 6), l'authentification, l'autorisation serveur (tenant/agence/rôle) et le CRUD `Tenant`/`Agency`/`Vehicle`/`Location`/`Client`/`Invoice`/`Payment`/`Maintenance` (Sprints 3–8), la prochaine étape reste conditionnée par les décisions encore ouvertes (section 8) — en particulier : confirmer les décisions prises en cours d'implémentation sans validation préalable distincte (granularité de rôle véhicules/locations, point 21 ; accès aux rapports réservé ADMIN, point 24 ; accès à `check-alerts` réservé ADMIN et scopé au tenant déclencheur, point 29), le choix d'un prestataire d'email transactionnel si les notifications par email sont confirmées comme nécessaires (point 30), le prochain module métier à construire (cautions ? incidents de location ?), et le flux d'invitation/gestion d'utilisateurs dans un tenant existant. Conformément à [CLAUDE.md](./CLAUDE.md), aucun code métier ou dépendance supplémentaire ne doit être ajouté sans validation explicite distincte de celle de ce sprint.
...
| `npm run test` | ✅ Validé | 146/146 tests passés (Vitest) — isolation multi-tenant/multi-agence, authentification, CRUD (tenants/agencies/vehicles/locations/clients/invoices/payments/maintenances), disponibilité/conflits, pricing, machine à états (locations + invoices + maintenances + alerts), numérotation de facture, recalcul automatique du statut de facture, rapports, génération idempotente d'alertes, acknowledge/resolve, suppression client bloquée si location associée, rendu des pages UI, aucune donnée résiduelle après nettoyage |
| `npm run build` | ✅ Validé | Build de production réussi (Turbopack, Next.js 16.3.0), TypeScript strict sans erreur, 31 routes API + 25 pages `/dashboard/*` + `/login`/`/register` + Proxy |
